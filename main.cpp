...
//デバッグ用
int8_t RX, RY, LX, LY;
uint8_t R2, L2, up, dwn, lft, rit, sqr, crss, cicl, tri, R1, L1, R3, L3, shr,
    opt, PSb, touc;
// const int RECIEVE_CAN_ID = 0x12;

const int RECIEVE_CAN_ID = 0x10; //コントローラーデバッグ時はコメントアウト
const int CAN_Hz =
    1000000; // CANに使用するクロック周波数[Hz]. CAN通信相手と共通させる
...
//エンコーダ（A層,B層,分解能）
Ec2multi ec[] = {
    Ec2multi(PC_3, PC_2, RESOLUTION), Ec2multi(PC_12, PC_10, RESOLUTION),
    Ec2multi(PB_14, PB_15, RESOLUTION), Ec2multi(PC_6, PC_8, RESOLUTION)};
//速度制御のPIDの値,角速度を計算する間隔,dutyの絶対値の上限
CalPID speed_pid[] = {CalPID(0.0, 0.0, 0.0, DELTA_T, DUTY_MAX),
...
int main() {
  ...
  can.filter(RECIEVE_CAN_ID, 0xFFF, CANStandard,
             0); ////MY_CAN_ID以外のデータを受け取らないよう設定．後半の0xFFF,
                 /// CANStandard, 0);は基本変えない
...
void unzipControl() {
  ...
  if (can.read(msg)) {
    ...
    L2 = msg.data[5];
    up =
        (msg.data[6] & 0b10000000) >>
        7; // rx_buf[6]の値はボタン8個の値を合成したものだから、ビットシフトで1桁に解凍
